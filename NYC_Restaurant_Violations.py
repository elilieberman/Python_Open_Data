...
cuis_viol.head()
#%% Work in Progress, correlation of violations to closed status
X = np.array(cuis_viol.drop(['closed'],1))
y = np.array(cuis_viol['closed'])
#X = preprocessing.scale(X)
cuis_viol.dropna(inplace = True)
y = np.array(cuis_viol['closed'])
print(len(X),len(y))
X_train, X_test, y_train, y_test = cross_validation.train_test_split(X,y, test_size = 0.2)
clf = svm.SVR()  # SVR classifier
#clf = LinearRegression() #fir model using LinReg
#clf = svm.SVR(kernel = 'poly')  # SVR classifier with kernel parameter
clf.fit(X_train,y_train) #fit the model using the training data
accuracy = clf.score(X_test, y_test) # test the fit 
